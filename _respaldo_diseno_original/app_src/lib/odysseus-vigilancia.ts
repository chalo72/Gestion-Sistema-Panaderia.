...
export const promptEscaneoOdysseus = (nombreCamara: string, reglas: ReglaOdysseus[]): string => {
  return [
    ...
    'Tareas pendientes del checklist de hoy (por si los ves inactivos):',
    getTareasPendientesHoyString(),
...
/** Clasifica gravedad de un texto de respuesta de Odysseus */
export const clasificarGravedad = (texto: string): 'critica' | 'alta' | 'normal' => {
  const t = texto.toLowerCase();
  if (t.includes('alerta crítica') || t.includes('alerta critica') || t.includes('🚨') || t.includes('sustraccion') || t.includes('robo') || t.includes('bolsillo') || t.includes('sin cobrar')) return 'critica';
  if (t.startsWith('alerta') || t.includes('alerta:') || t.includes('⚠️') || t.includes('atención:')) return 'alta';
  return 'normal';
};
