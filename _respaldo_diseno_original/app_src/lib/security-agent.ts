...
export function registrarIncidente(incidente: Omit<IncidenteVendedor, 'id' | 'fecha' | 'revisado'>): IncidenteVendedor {
  ...
  saveIncidentes([nuevo, ...todos]);
    ...
    // Enviar webhook a NEXUS (Antigravity)
    fetch('http://localhost:3005/api/alert', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(nuevo) }).catch(() => {});
