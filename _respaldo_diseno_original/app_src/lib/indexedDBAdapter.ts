...
export class IndexedDBAdapter implements DatabaseAdapter {
  ...
  /** Abre (o crea) la base de datos y registra todos los Object Stores. Idempotente y a prueba de bloqueos. */
  async init(): Promise<void> {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      ...
      // 🛡️ Safe guard: Si IndexedDB tarda más de 2.5s en responder (pestañas bloqueadas o permisos), desbloquear
      const lockTimeout = setTimeout(() => {
        console.warn('⚠️ [IndexedDB]: init() excedió 2500ms. Desbloqueando app en modo seguro...');
        safeResolve();
      }, 2500);
      ...
      try {
        ...
        request.onblocked = () => {
          console.warn('⚠️ [IndexedDB]: Base de datos bloqueada por otra pestaña abierta. Desbloqueando...');
          safeResolve();
        };
        ...
        request.onsuccess = (event) => {
          ...
          // Si otra pestaña intenta actualizar la versión, cerrar esta conexión para no bloquearla
          this.db.onversionchange = () => {
            console.warn('⚠️ [IndexedDB]: Cambio de versión detectado en otra pestaña. Liberando conexión.');
            this.db?.close();
            this.db = null;
        ...
        request.onerror = (event) => {
          ...
          safeResolve(); // Resolver en lugar de colgar la app
        };
      } catch (err) {
        console.error('❌ [IndexedDB]: Excepción abriendo IndexedDB:', err);
        safeResolve();
      }
    });
  ...
  /** Devuelve la instancia de DB asegurada (o null si está en fallback seguro). */
  private getDB(): IDBDatabase | null {
    return this.db;
  }
  ...
  private transaction(
    ...
    return new Promise((resolve, reject) => {
      const db = this.getDB();
      if (!db) {
        console.warn(`⚠️ [IndexedDB]: Base de datos no disponible para '${collection}'. Devolviendo fallback.`);
