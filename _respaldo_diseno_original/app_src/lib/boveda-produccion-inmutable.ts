...
import { DATOS_EJEMPLO } from './seed-data';
...
export const BOVEDA_FORMULACIONES: FormulacionBase[] = Object.freeze(
  JSON.parse(JSON.stringify(DATOS_EJEMPLO.formulaciones || []))
) as FormulacionBase[];

export const BOVEDA_MODELOS_PAN: ModeloPan[] = Object.freeze(
  JSON.parse(JSON.stringify(DATOS_EJEMPLO.modelosPan || []))
) as ModeloPan[];

// Identificadores de las 5 Masas Maestras Críticas que jamás pueden borrarse ni degradarse
export const MASAS_CRITICAS_MAP: Record<string, { nombre: string; minIngredientes: number }> = {
  '7d8e9f0a-b1c2-4d3e-8f9a-0b1c2d3e4f51': { nombre: 'Masa de Sal Mixta', minIngredientes: 11 },
  '7d8e9f0a-b1c2-4d3e-8f9a-0b1c2d3e4f52': { nombre: 'Masa de Dulce', minIngredientes: 11 },
  '7d8e9f0a-b1c2-4d3e-8f9a-0b1c2d3e4f53': { nombre: 'Masa de Hojaldre Mixta', minIngredientes: 8 },
  '7d8e9f0a-b1c2-4d3e-8f9a-0b1c2d3e4f54': { nombre: 'Batido de Tortas Maestro', minIngredientes: 11 },
  '7d8e9f0a-b1c2-4d3e-8f9a-0b1c2d3e4f55': { nombre: 'Vatido Galleta', minIngredientes: 8 },
};
...
export function blindarYRepararFormulaciones(actuales: FormulacionBase[]): {
  ...
  for (const bovedaForm of BOVEDA_FORMULACIONES) {
    ...
    const actual = mapActuales.get(bovedaForm.id);

    if (!actual) {
      // Masa crítica faltante: reinsertar desde la bóveda
      mapActuales.set(bovedaForm.id, { ...bovedaForm });
      reparaciones++;
      ...
      if (cantActual < minExigido) {
        // Masa degradada: restaurar ingredientes de la bóveda
        mapActuales.set(bovedaForm.id, {
          ...actual,
          ingredientes: [...(bovedaForm.ingredientes || [])],
