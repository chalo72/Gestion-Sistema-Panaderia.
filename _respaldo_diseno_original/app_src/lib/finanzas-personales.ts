...
import { localAdapter, db } from '@/lib/database';
...
export function addVentaDiaria(data: Omit<VentaDiaria, 'id' | 'total'> & { id?: string }): VentaDiaria {
  ...
  // Sincronizar silenciosamente a IndexedDB / Nube
  localAdapter.setDocument('ventas_diarias', nueva.id, nueva).catch(() => {});
...
export function deleteVentaDiaria(id: string): void {
  ...
  // Eliminar de IndexedDB / Nube
  localAdapter.deleteDocument('ventas_diarias', id).catch(() => {});
}
...
export function saveProducciones(list: RegistroProduccion[]): void {
  ...
  normalizados.sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''));
  const payload = normalizados.slice(0, 365);
  localStorage.setItem(KEY_PRODUCCIONES, JSON.stringify(payload));
  try {
    window.dispatchEvent(new Event('dp_producciones_changed'));
  } catch { /* ignore */ }

  // Respaldo asíncrono en IndexedDB y Supabase para sincronización PC <-> Celular
  void (async () => {
    try {
      await db.saveBackup('dp_producciones_diarias', payload);
    } catch (e) {
      console.warn('⚠️ [Producción] Error respaldando producciones en nube:', e);
    }
  })();
}

/**
 * Descarga y fusiona las producciones desde la nube (Supabase / IndexedDB) hacia el dispositivo actual.
 * Permite que lo anotado en PC aparezca inmediatamente en el celular.
 */
export async function sincronizarProduccionesNube(): Promise<number> {
  try {
    const remotas = await db.getBackup('dp_producciones_diarias');
    if (!remotas || !Array.isArray(remotas) || remotas.length === 0) return 0;

    const locales = getProducciones();
    const mapa = new Map<string, RegistroProduccion>();

    for (const p of remotas) {
      if (p?.id) mapa.set(p.id, { ...p, fecha: normalizarFechaYYYYMMDD(p.fecha) });
    }
    for (const p of locales) {
      if (p?.id) {
        // Si ya existe en la nube y local, conservamos el que tenga más campos o detalles
        const remoto = mapa.get(p.id);
        if (!remoto || ((p.hornadas?.length || 0) >= (remoto.hornadas?.length || 0))) {
          mapa.set(p.id, { ...p, fecha: normalizarFechaYYYYMMDD(p.fecha) });
        }
      }
    }

    const fusion = [...mapa.values()].sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''));
    if (fusion.length > locales.length) {
      localStorage.setItem(KEY_PRODUCCIONES, JSON.stringify(fusion.slice(0, 365)));
      try { window.dispatchEvent(new Event('dp_producciones_changed')); } catch { /* ignore */ }
      return fusion.length - locales.length;
    }
    return 0;
  } catch (err) {
    console.warn('⚠️ [Producción] Error sincronizando producciones desde nube:', err);
    return 0;
  }
}

/**
 * Recupera historial de producción desde claves de respaldo / nombres viejos en localStorage.
 * No borra lo actual: fusiona por id y vuelve a guardar en la clave oficial.
 */
export function recuperarProduccionesLocal(): { total: number; recuperados: number } {
  const existentes = getProducciones();
  const porId = new Map<string, RegistroProduccion>();
  for (const p of existentes) {
    if (p?.id) porId.set(p.id, p);
  }
  let recuperados = 0;
  const clavesExtra = [
    'dp_producciones_diarias_backup',
    'dp_producciones_backup',
    'producciones_diarias',
    'dp_auditorias_produccion',
    'pricecontrol_producciones',
  ];
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      const candidata =
        clavesExtra.includes(k) ||
        (lower.includes('produccion') && (lower.includes('dp_') || lower.includes('backup') || lower.includes('audit')));
      if (!candidata || k === KEY_PRODUCCIONES) continue;
      try {
        const raw = localStorage.getItem(k);
        if (!raw) continue;
        const parsed = JSON.parse(raw) as unknown;
        const lista = Array.isArray(parsed) ? parsed : (parsed && typeof parsed === 'object' && Array.isArray((parsed as { items?: unknown }).items) ? (parsed as { items: unknown[] }).items : null);
        if (!lista) continue;
        for (const item of lista) {
          if (!item || typeof item !== 'object') continue;
          const p = item as RegistroProduccion;
          if (!p.id || (!p.fecha && !(p as { masas?: unknown }).masas && !(p as { hornadas?: unknown }).hornadas)) continue;
          if (!porId.has(p.id)) {
            porId.set(p.id, { ...p, fecha: normalizarFechaYYYYMMDD(p.fecha) });
            recuperados += 1;
          }
        }
      } catch { /* ignore clave */ }
  ...
  const fusion = [...porId.values()].sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''));
  if (recuperados > 0 || fusion.length !== existentes.length) {
    saveProducciones(fusion);
  }
  return { total: fusion.length, recuperados };
}
...
// ── Auto-Migración y Recuperación de Nube ──────────────────────────────
(async () => {
  try {
    const fromDB = await localAdapter.getCollection<VentaDiaria>('ventas_diarias') || [];
    const local = getVentasDiarias();
    
    const dbIds = new Set(fromDB.map(v => v.id));
    const localIds = new Set(local.map(v => v.id));
    
    let localChanged = false;
    
    // 1. Nube -> Local (si falta en local, lo agregamos)
    fromDB.forEach(dbV => {
      if (!localIds.has(dbV.id)) {
        local.push(dbV);
        localChanged = true;
      }
    });
    ...
    if (localChanged) {
      local.sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime());
      saveVentasDiarias(local);
      window.dispatchEvent(new CustomEvent('dp-force-sync-ventas-diarias'));
    }
    
    // 2. Local -> Nube (si falta en IndexedDB, lo subimos)
    // Esto es CLAVE para rescatar los datos del navegador Brave y mandarlos a la nube.
    for (const localV of local) {
      if (!dbIds.has(localV.id)) {
        await localAdapter.setDocument('ventas_diarias', localV.id, localV);
      }
    }
  } catch (e) {
    console.error('Error in auto-migration:', e);
  }
})();
