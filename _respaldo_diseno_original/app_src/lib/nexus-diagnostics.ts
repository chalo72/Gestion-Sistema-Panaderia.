...
export const NexusDiagnostics = {
  ...
  /**
   * Envía telemetría al servidor Nexus Core (MCP Bridge) con cooldown anti-spam si está apagado
   */
  async sendTelemetry(type: string, data: any) {
    const now = Date.now();
    if (nexusOfflineCooldown && now - lastNexusAttempt < 60000) {
      return;
    }
    lastNexusAttempt = now;
    ...
    try {
      ...
      nexusOfflineCooldown = false;
    } catch {
      nexusOfflineCooldown = true;
      // Aviso silencioso único para evitar flood en consola
    }
  }
};

let nexusOfflineCooldown = false;
let lastNexusAttempt = 0;
