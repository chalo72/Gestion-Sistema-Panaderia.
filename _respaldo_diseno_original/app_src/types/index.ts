...
/** Zona visual de la factura de referencia (cabecera, tabla, pie). */
export interface ZonaFacturaPerfil {
  id: 'cabecera' | 'tabla' | 'pie';
  titulo: string;
  descripcion: string;
  lineaInicio: number;
  lineaFin: number;
...
/** Columna detectada en la tabla de ítems. */
export interface ColumnaFacturaPerfil {
  id: string;
  etiqueta: string;
  orden: number;
  presente: boolean;
...
/** Línea extraída de factura (3 campos mínimos). */
export interface LineaFacturaPerfil {
  descripcion: string;
  cantidad: number;
  unidad?: string;
  valorTotal: number;
...
/**
 * Perfil de reconocimiento de factura por proveedor (analogía: foto de rostro).
 * Se guarda en el proveedor; Gastos/Carga Masiva lo reutilizan al escanear.
 */
export interface PerfilFacturaProveedor {
  version: 1;
  activo: boolean;
  /** Imagen comprimida de la factura de referencia (data URL). */
  imagenReferencia?: string;
  tipoFactura: string;
  etiquetaTipo: string;
  nitDetectado?: string;
  razonSocialDetectada?: string;
  numeroFacturaEjemplo?: string;
  fechaFacturaEjemplo?: string;
  totalFacturaEjemplo?: number;
  zonas: ZonaFacturaPerfil[];
  columnas: ColumnaFacturaPerfil[];
  textosIgnorar: string[];
  reglasLectura: string[];
  calidadReferencia: number;
  lineasEjemploDetectadas: number;
  /** Líneas de producto extraídas al crear el perfil (descripción + cantidad + total). */
  lineasEjemplo?: LineaFacturaPerfil[];
  analizadoEn: string;
  confirmadoEn?: string;
}
...
export interface Proveedor {
  ...
  /** Plantilla de factura aprendida (reconocimiento por proveedor). */
  perfilFactura?: PerfilFacturaProveedor;
  createdAt: string;
}
...
export interface Usuario {
  ...
  password?: string;
  avatar?: string;
  /** Marca de tiempo para sync entre aparatos (LOCAL GANA si es más reciente). */
  updatedAt?: string;
  /** Vínculo opcional con ficha de trabajador. */
  trabajadorId?: string;
}
