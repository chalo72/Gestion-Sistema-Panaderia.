...
export function TurboExpenseManager({ 
  ...
  const [isSpreadsheet, setIsSpreadsheet] = useState(() => modoForzado === 'tabla');

  useEffect(() => {
    if (modoForzado === 'tabla') {
      setIsSpreadsheet(true);
    } else if (modoForzado === 'turbo') {
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto flex-wrap justify-end">
          ...
          {onOpenScanner && (
            <button 
              ...
              className="text-[9px] font-bold uppercase px-2 py-1 rounded-md transition-colors flex items-center gap-1 bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-200 border border-emerald-300 dark:border-emerald-800"
