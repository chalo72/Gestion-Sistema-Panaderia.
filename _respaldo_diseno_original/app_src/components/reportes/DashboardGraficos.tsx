...
import { fechaCalendarioLocalDesdeDate, fechaCalendarioLocalDesdeISO } from '@/lib/fecha-ventas';
...
export function DashboardGraficos({ gastos = [], ventasDiarias = [], ventasPOS = [] }: { gastos: any[], ventasDiarias: any[], ventasPOS: any[] }) {
    ...
    const data = useMemo(() => {
        ...
        if (rango === '15d' || rango === '30d') {
            ...
            for (let i = days - 1; i >= 0; i--) {
                ...
                const str = fechaCalendarioLocalDesdeDate(d);
                map.set(str, { 
                    fecha: str, 
        ...
        ventasPOS.forEach(v => {
            const dateStr = fechaCalendarioLocalDesdeISO(v.fecha);
            if (!dateStr) return;
        ...
        gastos.forEach(g => {
            ...
            const dateStr = fechaCalendarioLocalDesdeISO(g.fecha);
            if (!dateStr) return;
            const key = rango === '12m' ? dateStr.slice(0, 7) : dateStr;
            if (map.has(key)) {
    ...
    const gastosPorCategoria = useMemo(() => {
        ...
        const limitStr = fechaCalendarioLocalDesdeDate(limitDate);

        gastos.forEach(g => {
            if (g.esIngreso) return;
            const dStr = fechaCalendarioLocalDesdeISO(g.fecha);
            if (dStr >= limitStr) {
