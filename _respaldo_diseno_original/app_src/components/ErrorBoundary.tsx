...
export class ErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // Auto-impulso inmediato a la IA de telemetría
    enviarTelemetria(
      `CRASH_VISTA_${(this.props.moduleName || 'GLOBAL').toUpperCase().replace(/[^A-Z0-9_]/g, '_')}`,
      `Error en módulo "${this.props.moduleName || 'Desconocido'}": ${error.message}\nStack: ${error.stack?.slice(0, 300) || ''}`,
      'ErrorBoundary_React',
      { modulo: this.props.moduleName, componentStack: errorInfo.componentStack?.slice(0, 250) }
    );
