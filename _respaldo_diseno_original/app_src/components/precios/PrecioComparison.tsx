...
export function PrecioComparison({
    ...
    const getComparacion = (productoId: string) => {
        ...
        const ordenados = [...preciosProducto].sort((a, b) => costoUnit(a) - costoUnit(b));
        const masBarato = ordenados[0];
        const masCaro = ordenados[ordenados.length - 1];
        const diferencia = costoUnit(masCaro) - costoUnit(masBarato);
        // Ahorro real por unidad (kg/und), no por bulto
        const porcentaje = costoUnit(masCaro) > 0 ? (diferencia / costoUnit(masCaro)) * 100 : 0;
