...
import { fechaCalendarioLocalDesdeDate } from '@/lib/fecha-ventas';
...
export function AnalisisInteligente({
  ...
  const abrirEdicionSchedule = useCallback(() => {
    ...
    if (schedule?.proximaVisita) {
      ...
      setTempFecha(fechaCalendarioLocalDesdeDate(d));
    }
  ...
  const avanzarVisita = useCallback(() => {
    ...
    const data = { ...schedule, proximaVisita: fechaCalendarioLocalDesdeDate(base) };
    saveSchedule(proveedorId, data);
    setSchedule(data);
