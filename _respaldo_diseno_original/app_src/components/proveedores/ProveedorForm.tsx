...
export function ProveedorForm({
  ...
  const [showScannerBoveda, setShowScannerBoveda] = useState(false);
  ...
  // ── MOTOR DE RECONOCIMIENTO DE VOZ (MODO DIOS ULTRA) ──
  const toggleVoiceRecognition = () => {
    ...
    recognition.lang = 'es-CO'; // Optimizado para acentos locales (Colombia)
    recognition.continuous = true; // Escucha continua nativa
    recognition.interimResults = true; // Procesamiento en tiempo real
    recognition.maxAlternatives = 1;
    recognitionRef.current = recognition;

    isListeningRef.current = true;
    setIsListening(true);
    setVoiceFeedback('Escuchando (Modo Dios)...');
    toast.info('🎙️ Asistente activado.');
    ...
    recognition.onend = () => {
      if (isListeningRef.current) {
         // Auto-reinicio suave si se cortó por la API nativa (Chrome la corta a los 60s)
         setTimeout(() => {
            if (isListeningRef.current && recognitionRef.current) {
  ...
  const parseVoiceCommand = (text: string) => {
    ...
    if (cleanText.match(/(?:añade|agrega|guarda|registrar|guardar)/i)) {
      shouldAdd = true;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      ...
      >
          ...
          <div className="flex items-center gap-4">
             ...
             <div>
                ...
                <DialogDescription className="text-white/70 font-bold uppercase tracking-[0.2em] mt-1.5 text-[10px] flex items-center gap-2">
                  ...
                  {editingProveedor && (
                    <button type="button" onClick={() => setShowScannerBoveda(true)} className="mt-3 bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 border border-white/20">
                        <Camera className="w-4 h-4" /> Bóveda: Archivar Facturas ({formData.razonSocial})
                    </button>
                  )}
             </div>
          </div>
        ...
        <EscanerFacturaIA
            isOpen={showScannerBoveda}
            onClose={() => setShowScannerBoveda(false)}
            modo="PROVEEDOR"
            onScanComplete={async (res) => {
                setShowScannerBoveda(false);
                const toastId = toast.loading('Archivando en la Bóveda Digital...');
                try {
                    const prov = formData.razonSocial || 'Desconocido';
                    const url = await uploadFacturaGasto(res.archivo, prov, res.fecha);
                    if (url) {
                        toast.success(`Archivada: ${prov} / ${res.fecha?.slice(0,7) || 'Actual'}`, { id: toastId });
                    } else {
