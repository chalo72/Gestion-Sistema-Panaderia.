...
import { registrarVisitaVista } from '@/lib/navegacion-vigia';
...
/** Hidrata el badge desde la nube sin abrir Equipo (LOCAL sigue ganando si ya hay espejo). */
async function hidratarAnunciosDesdeNube(): Promise<void> {
  try {
    ...
    localStorage.setItem(
      'dp_anuncios',
      JSON.stringify(data.map((r: { created_at: string }) => ({ timestamp: r.created_at }))),
    );
    window.dispatchEvent(new Event('dp_anuncios_changed'));
  } catch {
    /* sin red: el badge usa lo local */
  }
}
...
export function Sidebar({
  ...
  useEffect(() => {
    ...
    void hidratarAnunciosDesdeNube();
    return () => {
  ...
  const allMenuGroups: MenuGroup[] = [
    {
      ...
      items: [
        ...
        { id: 'videovigilancia', label: 'Videovigilancia',     icon: Video,           permission: 'VER_DASHBOARD' },
          { id: 'expedientes',     label: 'Exped. Empleadas',    icon: Shield,          permission: 'VER_DASHBOARD' },
  ...
  const menuGroups = allMenuGroups
    .map(g => ({
      ...
      items: g.items.filter(i => {
        if (role === 'PANADERO' && (i.id === 'agentes-ia' || i.id === 'videovigilancia' || i.id === 'expedientes' || i.id === 'cctv' || i.id === 'comunicaciones')) {
          return false;
        }
