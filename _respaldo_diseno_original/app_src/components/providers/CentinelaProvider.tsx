...
export const CentinelaProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // [NEXUS TELEMETRY] Impulso de presencia y sincronización hacia el Daemon (puerto 3005)
  useEffect(() => {
    enviarTelemetria(
      'PLATAFORMA_EN_LINEA',
      `Pantalla activa: ${window.location.pathname || '/'} | Sync: ${syncConnected ? 'Conectado' : 'Offline'}`,
      'Centinela_Provider'
    );
  }, [syncConnected]);
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      // getAgenteHallazgos(arg) filtra por agenteId — no pasar un número como límite
      const todos = await db.getAgenteHallazgos();
      const h = [...(todos || [])]
