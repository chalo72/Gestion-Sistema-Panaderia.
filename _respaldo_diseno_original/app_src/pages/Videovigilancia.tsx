...
import { nuevaFalta } from '@/lib/expediente-empleada';
import { enviarAlertaEmail } from '@/lib/notificaciones';
import { clasificarGravedad } from '@/lib/odysseus-vigilancia';
import { detectFacesInFrame } from '@/lib/face-recognition';
import {
  Camera,
  Shield,
  Search,
...
export const getHostGo2rtc = () =>
  typeof window !== 'undefined' && window.location.hostname ? window.location.hostname : 'localhost';

export const CAMARAS_DEFAULT: Camara[] = [
  {
    id: 'cctv-cam1',
    nombre: 'Cámara 1 · Mostrador / Caja',
    url: `/api/camara-proxy?target=${encodeURIComponent('http://admin:admin123@192.168.18.141/ISAPI/Streaming/channels/101/picture')}`,
    tipo: 'snapshot',
    activa: true,
  },
  {
    id: 'cctv-cam2',
    nombre: 'Cámara 2 · Producción / Hornos',
    url: `/api/camara-proxy?target=${encodeURIComponent('http://admin:admin123@192.168.18.141/ISAPI/Streaming/channels/201/picture')}`,
    tipo: 'snapshot',
    activa: true,
  },
  {
    id: 'cctv-cam3',
    nombre: 'Cámara 3 · Vitrina / Entrada',
    url: `/api/camara-proxy?target=${encodeURIComponent('http://admin:admin123@192.168.18.141/ISAPI/Streaming/channels/301/picture')}`,
    tipo: 'snapshot',
    activa: true,
  },
  {
    id: 'cctv-cam4',
    nombre: 'Cámara 4 · Almacén / Insumos',
    url: `/api/camara-proxy?target=${encodeURIComponent('http://admin:admin123@192.168.18.141/ISAPI/Streaming/channels/401/picture')}`,
    tipo: 'snapshot',
    activa: true,
  },
];

export const CAMARAS_PUENTE_GO2RTC = CAMARAS_DEFAULT;

/** Asegura que URLs locales o con credenciales pasen por el proxy de Vite con Digest Auth */
export const asegurarProxyUrl = (url: string) => {
  if (!url) return '';
  if (url.startsWith('/api/camara-proxy')) return url;
  if (url.includes('192.168.18.141') || (url.includes('192.168.') && url.includes('@'))) {
    return `/api/camara-proxy?target=${encodeURIComponent(url)}`;
  }
  return url;
};

const conBuster = (url: string) => {
  if (!url || url.startsWith('data:') || url.includes('stream.html') || url.includes(':1984')) return url;
  const cleanUrl = url.replace(/([?&])_t=\d+/, '');
  const sep = cleanUrl.includes('?') ? '&' : '?';
  return `${cleanUrl}${sep}_t=${Date.now()}`;
};

/**
 * 🛰️ Resuelve la URL final para visualización fluida:
 * Retorna iframe para streams go2rtc explícitos, o imagen snapshot vía proxy Digest seguro.
 */
export const resolverVisualizacionCamara = (cam: { url: string; nombre?: string; esDemo?: boolean }): { esStream: boolean; urlFinal: string } => {
  if (!cam.url) return { esStream: false, urlFinal: '' };
  if (cam.esDemo) return { esStream: false, urlFinal: cam.url };

  const urlLower = cam.url.toLowerCase();

  // Si explícitamente es un stream HTML de go2rtc en puerto 1984
  if (urlLower.includes(':1984/stream.html') || urlLower.endsWith('.html')) {
    const host = getHostGo2rtc();
    const finalUrl = cam.url.replace(/(localhost|127\.0\.0\.1|192\.168\.\d+\.\d+):1984/, `${host}:1984`);
    return { esStream: true, urlFinal: finalUrl };
  }

  // Snapshot normal a través de proxy seguro
  return { esStream: false, urlFinal: asegurarProxyUrl(cam.url) };
};
...
export function Videovigilancia() {
  ...
  const cargarCamaras = useCallback(async () => {
    try {
      ...
      if (vivas.length > 0) {
        // Auto-migrar cámaras si apuntan a go2rtc 1984 roto o DVR sin proxy
        const normalizadas = vivas.map((cam) => {
          let url = cam.url;
          if (url.includes(':1984/stream.html') || url.includes(':1984')) {
            if (cam.id === 'cctv-cam1' || url.includes('cam1') || cam.nombre.includes('1')) {
              url = CAMARAS_DEFAULT[0].url;
            } else if (cam.id === 'cctv-cam2' || url.includes('cam2') || cam.nombre.includes('2')) {
              url = CAMARAS_DEFAULT[1].url;
            } else if (cam.id === 'cctv-cam3' || url.includes('cam3') || cam.nombre.includes('3')) {
              url = CAMARAS_DEFAULT[2].url;
            } else if (cam.id === 'cctv-cam4' || url.includes('cam4') || cam.nombre.includes('4')) {
              url = CAMARAS_DEFAULT[3].url;
            }
          }
          const urlProxy = asegurarProxyUrl(url);
          const tipo = urlProxy.includes('stream.html') ? ('mjpeg' as const) : ('snapshot' as const);
          const actualizada = { ...cam, url: urlProxy, tipo };
          if (cam.url !== urlProxy || cam.tipo !== tipo) {
            db.saveCamara(actualizada).catch(() => {});
          }
          return actualizada;
        });
        setCamaras(normalizadas);
      } else {
        // Cargar las 4 cámaras por defecto del DVR
        for (const c of CAMARAS_DEFAULT) {
          db.saveCamara(c).catch(() => {});
        }
  ...
  const capturarFrame = (camaraId: string): string | null => {
    const img = imgRefs.current[camaraId];
    ...
    if (!isVideo && (!img.complete || img.naturalWidth === 0)) return null;
    if (isVideo && (img.readyState < 2 || img.videoWidth === 0)) return null;
  ...
  const refrescarSnapshot = (camaraId: string) => {
    ...
    if (img.tagName.toLowerCase() !== 'video') {
      ...
      const vis = resolverVisualizacionCamara(cam);
      img.src = conBuster(vis.urlFinal);
    }
  };
  ...
  const escanearCamara = useCallback(
    async (camara: Camara) => {
      ...
      try {
        ...
        const perfiles = await db.getAllEmpleadasPerfil();
        const perfilesConRostro = perfiles.filter(p => p.faceDescriptor).map(p => ({ id: p.id, nombre: p.nombre, descriptor: p.faceDescriptor as number[] }));
        
        let carasIdentificadas = '';
        let empleadaInfractoraId = 'sin_identificar';
        
        const imgRef = imgRefs.current[camara.id];
        if (imgRef && perfilesConRostro.length > 0) {
          try {
             const caras = await detectFacesInFrame(imgRef, perfilesConRostro);
             if (caras.length > 0) {
               carasIdentificadas = ' \nNOTA DEL SISTEMA BIOMÉTRICO: En esta imagen se identificó a: ' + caras.map(c => c.nombre).join(', ') + '. Trátalos por su nombre si cometen una falta.';
               empleadaInfractoraId = caras[0].id;
             }
          } catch(e) { console.error('Error biometrico', e); }
        }

        const mensajeFinal = mensaje + carasIdentificadas;
        const res = await consultarAgente('odysseus', mensajeFinal, () => {}, frame);
        ...
        const gravedadClas = clasificarGravedad(texto);

        agregarLog(`[${camara.nombre}] ${texto}`, esAlerta || sinImg);

        if (esAlerta && !sinImg) {
          const falta = nuevaFalta({
            empleadaId: empleadaInfractoraId,
            camaraId: camara.id,
            cameraNombre: camara.nombre,
            tipo: 'alerta_general',
            titulo: gravedadClas === 'critica' ? 'Alerta Crítica CCTV' : 'Alerta de Seguridad',
            descripcion: texto,
            gravedad: gravedadClas as 'leve'|'alta'|'critica',
            evidenciaFrame: frame,
          });
          await db.saveFaltaEmpleada(falta);

          await db.addAgenteHallazgo('odysseus', {
            id: generateUUID(),
            tipo: 'seguridad',
            gravedad: gravedadClas,
          ...
          toast.error(`ODYSSEUS: ${gravedadClas === 'critica' ? 'ALERTA CRÍTICA' : 'Alerta'} en ${camara.nombre}`);
          
          if (gravedadClas === 'critica' || gravedadClas === 'alta') {
             enviarAlertaEmail('admin@dulceplacer.com', 'Alerta Odysseus CCTV', texto, gravedadClas, camara.nombre, 'Sin Identificar');
          }
        }
      } catch (err) {
      ...
      <header className="shrink-0 border-b border-white/10 bg-slate-900/70 backdrop-blur-xl px-6 py-4 flex items-center justify-between z-20">
        ...
        <div className="flex items-center gap-3">
            <Button
              onClick={async () => {
                for (const c of CAMARAS_DEFAULT) {
                  await db.saveCamara(c).catch(() => {});
                }
                setCamaras(CAMARAS_DEFAULT);
                toast.success('4 Cámaras del DVR conectadas exitosamente');
              }}
              variant="outline"
              className="border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 h-10 px-3 rounded-xl text-xs font-bold"
              title="Restablece las 4 cámaras directamente al DVR local vía proxy seguro"
            >
              <RefreshCw className="w-3.5 h-3.5 mr-1.5 text-emerald-400" />
              Reconectar 4 Cámaras DVR
            </Button>
            <Button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {camaras.map((cam) => (
              <div
                key={cam.id}
                className={cn(
                  'cam-container bg-slate-900 border rounded-2xl overflow-hidden group',
              ...
              >
                <div className="relative aspect-video bg-black flex items-center justify-center overflow-hidden">
                  <div className="absolute top-3 left-3 z-10 flex items-center gap-2 px-2 py-1 bg-black/60 backdrop-blur-md rounded-md border border-white/10">
                    ...
                    <button
                      onClick={(e) => {
                        const container = e.currentTarget.closest('.cam-container');
                        if (container) {
                          if (document.fullscreenElement) document.exitFullscreen();
                          else container.requestFullscreen();
                        }
                      }}
                      className="absolute top-3 right-3 z-20 p-1.5 bg-black/60 backdrop-blur-md rounded-md border border-white/10 text-white/70 hover:text-white hover:bg-black/80 transition-all opacity-0 group-hover:opacity-100"
                  ...
                  {(() => {
                    ...
                    if (vis.esStream) {
                      return (
                        <iframe
                          ...
                          title={cam.nombre}
                          className={cn(
                            'w-full h-full border-0 transition-all duration-300 pointer-events-auto bg-black',
                    ...
                    return (
                      <img
                        ref={(el) => {
                          imgRefs.current[cam.id] = el;
                        }}
                        src={conBuster(vis.urlFinal)}
                        className={cn(
                        ...
                        onError={(e) => {
                          ...
                          if (!cam.esDemo && cam.activa) {
                            setTimeout(() => {
                              const img = imgRefs.current[cam.id];
                              if (img) img.src = conBuster(vis.urlFinal);
                            }, 3000);
                          }
                        }}
                        onLoad={(e) => {
        ...
        <div className="w-96 bg-black/60 backdrop-blur-2xl border-l border-white/5 flex flex-col z-20">
          ...
          <div className="shrink-0 p-4 border-b border-white/5 space-y-2 bg-emerald-500/5">
            ...
            <select
              ...
              className="w-full bg-black/50 border border-white/10 rounded-lg px-2 py-1.5 text-[11px] text-white focus:outline-none focus:border-emerald-500/40"
            >
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <button
                  key={q}
                  onClick={() => setPreguntaIa(q)}
                  className="text-[9px] px-2 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 hover:bg-emerald-500/25 transition-colors truncate max-w-full"
                >
                  {q}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Escribe cualquier pregunta sobre la cámara..."
                disabled={preguntando}
                className="flex-1 bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-500/50 disabled:opacity-50"
              />
              <Button
                type="button"
                disabled={preguntando}
                onClick={() => void preguntarAOdysseus()}
                className="h-9 w-9 shrink-0 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black disabled:opacity-50"
              >
                {preguntando ? (
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            ...
            {bitacora.map((log) => {
              const esCritico = log.mensaje.toLowerCase().includes('alerta crítica') ||
                log.mensaje.toLowerCase().includes('alerta critica') ||
                log.mensaje.toLowerCase().includes('🚨');
              const esPregunta = log.mensaje.toLowerCase().startsWith('[tú') ||
                log.mensaje.toLowerCase().startsWith('[tu');
              return (
                <div
                  key={log.id}
                  className={cn(
                    'p-3 rounded-xl border text-[12px] leading-relaxed transition-all animate-ag-fade-in',
                    esCritico
                      ? 'bg-red-600/15 border-red-500/50 text-red-100 shadow-lg shadow-red-500/10'
                      : log.alerta
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5 opacity-70">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest">
                      {log.tiempo.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true })}
                    </span>
                    {esCritico && (
                      <span className="ml-auto text-[8px] bg-red-500 text-white px-1.5 py-0.5 rounded-full font-black uppercase tracking-wider animate-pulse">
                        CRÍTICO
                      </span>
                    )}
                  </div>
                  <p className="font-medium leading-snug">{log.mensaje}</p>
                </div>
              );
            })}
          </div>
