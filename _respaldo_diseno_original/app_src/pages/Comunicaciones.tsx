...
export default function Comunicaciones() {
  ...
  const toggleTarea = (tareaId: string) => {
    ...
    if (yaCompletada) {
      ...
      const tareaObj = TAREAS_DEFAULT.find(t => t.id === tareaId);
      fetch('http://localhost:3005/api/alert', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tipoIncidente: 'CHECKLIST_CUMPLIDO',
          descripcion: `Tarea cumplida: ${tareaObj?.texto || tareaId} [Momento: ${tareaObj?.momento || 'general'}]`,
          vendedorNombre: `${usuario.nombre} (${rol})`
        })
      }).catch(() => {});
    }
  ...
  const registrarAsistencia = (tipo: 'entrada' | 'salida') => {
    ...
    fetch('http://localhost:3005/api/alert', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        tipoIncidente: tipo === 'entrada' ? 'TURNO_LLEGADA' : 'TURNO_SALIDA',
        descripcion: `Registro de turno: ${tipo.toUpperCase()} marcado a las ${fmtHora(nuevo.timestamp)}`,
        vendedorNombre: nuevo.usuarioNombre
      })
    }).catch(() => {});
  };
