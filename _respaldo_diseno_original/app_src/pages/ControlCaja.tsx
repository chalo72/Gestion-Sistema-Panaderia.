...
function EntregaTurnoModal({ caja, isOpen, onClose, onConfirmar, formatCurrency }: EntregaModalProps) {
    ...
    const handleConfirmar = async () => {
        ...
        fetch('http://localhost:3005/api/alert', {
            ...
            body: JSON.stringify({
                tipoIncidente: hayAlerta ? 'DESCUADRE_CAJA_CRITICO' : esFaltante ? 'DESCUADRE_CAJA_MENOR' : 'ENTREGA_TURNO_OK',
                descripcion: `Entrega de turno ${caja.turno} (${caja.cajaNombre}): Esperado $${esperado.toLocaleString('es-CO')} | Entregado $${entregado.toLocaleString('es-CO')} | Diferencia: $${diferencia.toLocaleString('es-CO')}`,
                vendedorNombre: caja.vendedoraNombre || 'Vendedora'
            })
        }).catch(() => {});
        setMonto('');
        setLoading(false);
...
interface ControlCajaProps {
    ...
    /** Nombre canónico del hook */
    registrarMovimientoCaja?: (monto: number, tipo: 'entrada' | 'salida', motivo: string, usuarioId: string, cajaId?: string) => Promise<unknown>;
    /** Alias legacy (App puede pasar ambos) */
    onRegistrarMovimiento?: (monto: number, tipo: 'entrada' | 'salida', motivo: string, usuarioId: string, cajaId?: string) => Promise<unknown>;
    usuario?: { id?: string; nombre?: string; rol?: string } | null;
    categorias?: Categoria[];
    productos?: Producto[];
    onAbrirCaja: (monto: number) => Promise<unknown>;
    onCerrarCaja: (monto: number) => Promise<unknown>;
}

export function ControlCaja({
    sesiones, ventas, cajaActiva, formatCurrency,
    getProductoById, registrarMovimientoCaja, onRegistrarMovimiento, usuario,
    onAbrirCaja, onCerrarCaja
}: ControlCajaProps) {
    const registrarMov = registrarMovimientoCaja || onRegistrarMovimiento;
    ...
    const registrarPrestamo = async (params: {
        ...
        if (!registrarMov) {
            toast.error('No se pudo registrar el movimiento de caja (falta cableado).');
            return;
        }
        await registrarMov(params.monto, 'salida',
            `Préstamo a ${params.cajaDestinoNombre}`, usuario?.id || 'admin', params.cajaOrigenId);
        await registrarMov(params.monto, 'entrada',
    ...
    const marcarDevuelto = async (prestamo: PrestamoEntreCajas) => {
        ...
        if (registrarMov) {
            await registrarMov(prestamo.monto, 'entrada',
                `Devolución de ${prestamo.cajaDestinoNombre}`, usuario?.id || 'admin', prestamo.cajaOrigenId);
            await registrarMov(prestamo.monto, 'salida',
    ...
    const ventasHoy = useMemo(() => filtrarRegistrosDelDiaLocal(ventas), [ventas]);

    const totalVentasHoy = useMemo(() =>
        ventasHoy.reduce((acc, v) => acc + v.total, 0)
    , [ventasHoy]);

    /** Solo billetes/monedas del cajón (no Nequi/tarjeta/crédito) */
    const totalEfectivoCajonHoy = useMemo(() =>
        ventasHoy.filter((v) => v.metodoPago === 'efectivo').reduce((acc, v) => acc + v.total, 0)
    , [ventasHoy]);
    ...
    const ventasHoyCaja = useMemo(() =>
        ventasHoy.filter(v => {
            if (!cajaVista) return true;
            if (v.cajaId === cajaVista.id) return true;
            return !v.cajaId && (cajaVista.ventasIds || []).includes(v.id);
        })
    , [ventasHoy, cajaVista]);
    ...
    const totalEfectivoCajonVista = useMemo(() =>
        ventasHoyCaja.filter((v) => v.metodoPago === 'efectivo').reduce((a, v) => a + v.total, 0)
    , [ventasHoyCaja]);
    ...
    const balanceEsperadoVista = (cajaVista?.montoApertura || 0) + totalEfectivoCajonVista + totalEntradasVista - totalSalidasVista;
    ...
    const balanceEsperado  = (cajaActiva?.montoApertura || 0) + totalEfectivoCajonHoy + totalEntradas - totalSalidas;
        ...
        <div className="min-h-full flex flex-col gap-5 p-4 bg-slate-50 dark:bg-slate-950 animate-ag-fade-in">
            ...
            <CajaMovimientosModal
                ...
                onSubmit={async (monto, motivo, cajaId) => {
                    if (!registrarMov) {
                        toast.error('No se pudo registrar el movimiento (falta cableado desde App).');
                        return;
                    }
                    await registrarMov(monto, movementModal.tipo, motivo, usuario?.id || 'anon', cajaId);
                }}
            />
