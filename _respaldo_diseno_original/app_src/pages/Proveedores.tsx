...
import { EscanerFacturaIA } from '@/components/agentes/EscanerFacturaIA';
import { uploadFacturaGasto } from '@/lib/storage-service';
import { PerfilFacturaSection } from '@/components/proveedores/PerfilFacturaSection';
import { proveedorTienePerfilFactura } from '@/lib/factura-perfil';
...
function ProveedorAvatar({ proveedor, size = 'md', className }: { proveedor: Proveedor; size?: 'sm' | 'md' | 'lg'; className?: string }) {
  const cls = size === 'lg'
    ? 'w-16 h-16 sm:w-24 sm:h-24 text-2xl sm:text-4xl rounded-2xl sm:rounded-[2rem]'
    : size === 'sm'
      ? 'w-9 h-9 sm:w-10 sm:h-10 text-sm sm:text-base rounded-lg sm:rounded-xl'
      : 'w-12 h-12 sm:w-16 sm:h-16 text-xl sm:text-3xl rounded-xl sm:rounded-[1.5rem]';
  return proveedor.imagen ? (
    <img src={proveedor.imagen} alt={proveedor.nombre} className={cn(cls, 'object-cover border-2 border-white/20 shadow-xl', className)} />
  ) : (
    <div className={cn(cls, 'bg-gradient-to-br from-blue-600 to-indigo-700 text-white flex items-center justify-center font-black border-2 border-white/10 shadow-xl', className)}>
...
type TabDetalle = 'factura' | 'contacto' | 'catalogo' | 'analisis';
...
export function Proveedores({
  ...
  const [bovedaScannerProvId, setBovedaScannerProvId] = useState<string | null>(null);
  ...
  const recuperarProveedor = useCallback(async (idx: number) => {
    ...
    try {
      ...
      // LOCAL GANA: si ya está aquí, no pisar — solo completar precios que falten por id
      if (entry.yaExisteLocal) {
        const idsLocales = new Set(getPreciosByProveedor(p.id).map((pr) => pr.id));
        let agregados = 0;
        for (const precio of entry.precios) {
          if (idsLocales.has(precio.id)) continue;
          await db.removeTombstone('precios', precio.id).catch(() => {});
          await db.addPrecio({
            id: precio.id,
        ...
        toast.success(
          agregados > 0
            ? `Se agregaron ${agregados} precio(s) faltantes (sin tocar lo que ya tenías).`
            : `"${p.nombre}" ya estaba completo aquí. No se cambió nada.`
        );
        if (agregados > 0) setNecesitaRecargar(true);
        setProveedoresEliminados(prev =>
  ...
  }, [proveedoresEliminados, getPreciosByProveedor]);
  ...
  return (
    <div className="min-h-full flex flex-col gap-5 p-4 bg-slate-50 dark:bg-slate-950 animate-ag-fade-in">
        ...
        <div className="flex flex-col gap-2">
          ...
          {alertasVisita.map(({ prov, alerta }) => (
            <div key={prov.id} className={cn(
              ...
              <p className="text-[11px] sm:text-xs font-bold sm:font-black text-slate-800 dark:text-white flex-1 leading-snug line-clamp-3 sm:line-clamp-none">
                {alerta.diasRestantes <= 0
                  ? <>🚨 Hoy llega el preventista de <span className="font-black text-slate-900 dark:text-white">{prov.nombre}</span> — ¡prepedido listo?</>
                  : alerta.diasRestantes === 1
                  ? <>🔔 Mañana llega el preventista de <span className="font-black text-slate-900 dark:text-white">{prov.nombre}</span> — prepara el prepedido</>
                  : <>📅 En {alerta.diasRestantes} días llega el preventista de <span className="font-black text-slate-900 dark:text-white">{prov.nombre}</span></>}
              </p>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pb-10">
        {filtrados.length === 0 ? (
          ...
          <div className={cn(
            'gap-2 sm:gap-3',
            vistaActual === 'grid'
              ? 'grid grid-cols-1 min-[400px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5'
              : 'flex flex-col'
          )}>
            {filtrados.map(prov => {
              ...
              return (
                <div key={prov.id} id={`prov-${prov.id}`} className={cn(
                  ...
                  <button
                    type="button"
                    className={cn(
                      'w-full flex items-center sm:items-start gap-2 sm:gap-3 cursor-pointer group text-left',
                      vistaActual === 'lista' ? 'px-3 sm:px-5 py-2.5 sm:py-3' : 'p-3',
                    )}
                    onClick={() => {
                  ...
                  >
                    ...
                    <div className="relative shrink-0 mt-0.5">
                      ...
                      <div className={cn('absolute -bottom-0.5 -right-0.5 w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full border-2 border-white dark:border-slate-800', activo ? 'bg-emerald-500' : 'bg-slate-300')} />
                    </div>
                    {/* Nombre + rubro */}
                    <div className="min-w-0 flex-1">
                      {rubro && (
                        <p className="text-[8px] sm:text-[9px] font-black text-violet-500 uppercase tracking-widest truncate">
                          {rubro}
                        </p>
                      )}
                      <p
                        className={cn(
                          'font-semibold sm:font-black leading-tight sm:leading-snug transition-colors text-slate-800 dark:text-white group-hover:text-blue-600',
                          'text-[10px] sm:text-sm',
                          'line-clamp-2 sm:line-clamp-1',
                          'break-words hyphens-auto normal-case tracking-normal',
                          isSel && vistaActual === 'grid' ? 'text-blue-700 dark:text-blue-400' : '',
                        )}
                        title={prov.nombre}
                      >
                        {prov.nombre}
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          {totalInsumos > 0 && (
                            <span className="text-[9px] font-black text-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 px-1.5 py-0.5 rounded">
                              <FlaskConical className="w-2 h-2 inline mr-0.5" />{totalInsumos}
                            </span>
                          )}
                          {totalVenta > 0 && (
                            <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded">
                              <Store className="w-2 h-2 inline mr-0.5" />{totalVenta}
                            </span>
                          )}
                        </div>
                      )}
                      {/* En lista móvil: badges compactos debajo del nombre */}
                      {vistaActual === 'lista' && (
                        <div className="flex md:hidden items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          {totalInsumos > 0 && (
                            <span className="text-[8px] font-black text-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 px-1.5 py-0.5 rounded-full">
                              {totalInsumos} ins.
                            </span>
                          )}
                          {totalVenta > 0 && (
                            <span className="text-[8px] font-black text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 px-1.5 py-0.5 rounded-full">
                              {totalVenta} vta.
                            </span>
                          )}
                        </div>
                    ...
                    {/* En lista escritorio: contacto + badges en línea */}
                    {vistaActual === 'lista' && (
                      <>
                        <div className="hidden md:flex items-center gap-5 shrink-0 text-xs">
                          ...
                          {prov.email && <span className="text-slate-400 flex items-center gap-1 max-w-[180px] truncate"><Mail className="w-3 h-3 shrink-0" />{prov.email}</span>}
                        </div>
                    ...
                    <ChevronDown className={cn('w-4 h-4 shrink-0 mt-1 transition-transform duration-300', isSel ? 'rotate-180 text-blue-500' : 'text-slate-300 group-hover:text-slate-500')} />
                  </button>
                  ...
                  {isSel && (
                    <div className="border-t-2 border-blue-100 dark:border-blue-800/40 bg-white dark:bg-slate-900">
                      ...
                      <div className="px-3 sm:px-6 py-4 sm:py-5 bg-gradient-to-r from-blue-50 to-indigo-50/30 dark:from-blue-900/20 dark:to-indigo-900/10 border-b border-blue-100 dark:border-blue-800/30 flex flex-col sm:flex-row items-start gap-3 sm:gap-5">
                        <ProveedorAvatar proveedor={prov} size="lg" className="mx-auto sm:mx-0" />
                        <div className="min-w-0 flex-1 space-y-2 w-full">
                          <div className="text-center sm:text-left">
                            {rubro && <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-violet-500 mb-0.5 truncate">{rubro}</p>}
                            <h3
                              className="text-sm sm:text-lg font-bold sm:font-black leading-snug text-slate-800 dark:text-white line-clamp-3 sm:line-clamp-2 break-words"
                              title={prov.nombre}
                            >
                              {prov.nombre}
                            </h3>
                            <div className="flex items-center justify-center sm:justify-start gap-1 mt-1">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star key={i} className={cn('w-3 h-3 sm:w-3.5 sm:h-3.5', i < (prov.calificacion || 5) ? 'fill-amber-400 text-amber-400' : 'text-slate-200')} />
                              ))}
                              <span className="text-[10px] sm:text-xs font-black text-amber-500 ml-1">{prov.calificacion || 5}.0</span>
                            </div>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-xs sm:text-sm mt-1">
                            {prov.telefono && (
                        ...
                        {/* Acciones */}
                        <div className="grid grid-cols-2 sm:flex sm:flex-col gap-2 shrink-0 w-full sm:w-auto">
                          {prov.telefono && (
                            <button onClick={() => window.open(`https://wa.me/${(prov.telefono||'').replace(/\D/g,'')}`, '_blank')}
                              className="h-9 px-2 sm:px-4 flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] sm:text-xs font-black transition-colors">
                              <MessageCircle className="w-3.5 h-3.5" /> WhatsApp
                            </button>
                          )}
                          <button onClick={() => handleEdit(prov)}
                            className="h-9 px-2 sm:px-4 flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[10px] sm:text-xs font-black transition-colors">
                            <Edit2 className="w-3.5 h-3.5" /> Editar
                          </button>
                          <button onClick={() => setBovedaScannerProvId(prov.id)}
                            className="h-9 px-2 sm:px-4 flex items-center justify-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[10px] sm:text-xs font-black transition-colors">
                            <Camera className="w-3.5 h-3.5" /> Archivar
                          </button>
                          <button onClick={() => descargarCatalogoPdf(prov)}
                            className="h-9 px-2 sm:px-4 flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] sm:text-xs font-black transition-colors">
                            <Download className="w-3.5 h-3.5" /> Catálogo
                          </button>
                          <button onClick={() => confirmarEliminar(prov)}
                            className="h-9 px-2 sm:px-4 flex items-center justify-center gap-1.5 text-rose-400 hover:text-white hover:bg-rose-600 border border-rose-200 dark:border-rose-800 rounded-xl text-[10px] sm:text-xs font-black transition-colors">
                      ...
                      {/* ── Tabs sutiles ── */}
                      <div className="flex gap-1 sm:gap-1.5 px-2 sm:px-6 py-2 bg-slate-50/80 dark:bg-slate-900/80 border-b border-slate-100 dark:border-slate-800 overflow-x-auto scrollbar-hide">
                        {([
                          { id: 'factura' as TabDetalle, label: 'Reconoc.', short: 'Rec.', icon: Fingerprint, badge: proveedorTienePerfilFactura(prov.perfilFactura) },
                          { id: 'catalogo' as TabDetalle, label: `Catálogo (${totalInsumos}🏭 + ${totalVenta}🛒)`, short: `Cat. ${totalInsumos + totalVenta}`, icon: Package },
                          { id: 'analisis' as TabDetalle, label: 'Análisis', short: 'Anál.', icon: BarChart3 },
                          { id: 'contacto' as TabDetalle, label: 'Contacto', short: 'Cont.', icon: UserCheck },
                          ...
                          <button
                            ...
                            className={cn(
                              'flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all shrink-0',
                              tabDetalle === id
                          ...
                          >
                            ...
                            {badge && (
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0" title="Perfil de factura activo" />
                            )}
                          </button>
                        ))}
                      </div>

                      {/* ── Tab: Reconocimiento de factura ── */}
                      {tabDetalle === 'factura' && (
                        <PerfilFacturaSection
                          proveedor={prov}
                            ...
                            <div className="rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-700 text-white p-5 relative overflow-hidden">
                              <div className="flex items-start justify-between gap-4 flex-wrap">
                                <div>
                                  ...
                                  <h4 className="text-sm sm:text-lg font-bold sm:font-black leading-snug line-clamp-2">{prov.nombre}</h4>
                  ...
                  <div key={entry.proveedor.id} className={`flex items-center justify-between gap-3 p-4 rounded-2xl border ${entry.precios.length > 0 ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800'}`}>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-bold sm:font-black text-slate-800 dark:text-white text-[11px] sm:text-sm leading-snug line-clamp-2 normal-case break-words">{entry.proveedor.nombre}</p>
                        {entry.yaExisteLocal && (
                    ...
                    >
                      {entry.recuperando
                        ...
                        : entry.yaRecuperado
                          ...
                          : <><RotateCcw className="w-3 h-3 mr-1" />{entry.yaExisteLocal ? 'Completar faltantes' : 'Restaurar'}</>
                      }
                    </Button>
                  </div>
                ))}
      ...
      {/* Escáner de Bóveda para el proveedor seleccionado */}
      <EscanerFacturaIA
        isOpen={!!bovedaScannerProvId}
        onClose={() => setBovedaScannerProvId(null)}
        modo="PROVEEDOR"
        titulo="Bóveda Digital"
        descripcion="Sube la foto para archivarla. Solo leeré la fecha y guardaré la imagen en la carpeta del proveedor sin alterar inventarios."
        onScanComplete={async (res) => {
            const provId = bovedaScannerProvId;
            setBovedaScannerProvId(null);
            
            const provObj = proveedores.find(p => p.id === provId);
            const provNombre = provObj?.razonSocial || provObj?.nombre || 'Desconocido';
            
            const toastId = toast.loading('Archivando en la Bóveda Digital...');
            try {
                const url = await uploadFacturaGasto(res.archivo, provNombre, res.fecha);
                if (url) {
                    toast.success(`Archivada correctamente: ${provNombre}`, { id: toastId });
                } else {
                    toast.error('Error al subir a la bóveda', { id: toastId });
                }
            } catch {
                toast.error('Error al subir a la bóveda', { id: toastId });
            }
