...
export default function RoleManager({ publicAppUrl }: RoleManagerProps) {
    ...
    const TABS: { id: TabId; label: string; icon: any }[] = [
        ...
        { id: 'claves',    label: 'Usuarios y Claves',     icon: KeyRound },
    ];
    ...
    return (
        <div className="min-h-full flex flex-col gap-6 p-4 bg-slate-50 dark:bg-slate-950 animate-ag-fade-in pb-20">
            ...
            {activeTab === 'modulos' && (
                <Card className="border-none shadow-xl bg-gradient-to-br from-slate-900/5 to-card dark:from-slate-900/40 dark:to-card backdrop-blur-sm overflow-hidden relative">
                    ...
                    <CardContent className="space-y-5">
                        ...
                        <button
                            onClick={() => { guardarPermisos(permisosModulos); toast.success('Permisos de módulos guardados y sincronizados a todos los celulares'); }}
            ...
            {/* ══════════════════════════════════════════════════════════════
                TAB — USUARIOS Y CLAVES (fuente única)
            ══════════════════════════════════════════════════════════════ */}
            {activeTab === 'claves' && (
                <div className="space-y-4">
                    <Card className="border-none shadow-lg bg-indigo-50/80 dark:bg-indigo-950/30">
                        <CardContent className="p-5 text-sm text-slate-700 dark:text-slate-200">
                            <p className="font-black uppercase text-[10px] tracking-widest text-indigo-600 mb-2">
                                Una sola libreta de acceso
                            </p>
                            <p>
                                Las claves para entrar a la app se crean <strong>por persona</strong> (no por rol genérico).
                                Lo que guardes aquí se copia automáticamente a todos los celulares conectados.
                                Desde <strong>Equipo → Trabajadores</strong> puedes crear acceso vinculado a cada empleada.
                            </p>
                            <p className="text-xs text-muted-foreground mt-2">
                                El PIN de Seguridad (descuentos / anti-fraude) es otro candado distinto — no es la clave de login.
                            </p>
                        </CardContent>
                    </Card>
                    <Usuarios publicAppUrl={publicAppUrl} />
                </div>
            )}
            ...
            <div className="bg-indigo-600/5 border border-indigo-600/10 rounded-[2.5rem] p-8 backdrop-blur-md shadow-xl flex flex-col md:flex-row items-center gap-6">
                ...
                <div className="flex-1 text-center md:text-left space-y-1">
                    ...
                    <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest leading-loose opacity-70">
                        Los cambios de permisos y módulos se copian en tiempo real a todos los aparatos conectados.
