...
export function Precios({
  ...
  const filteredPrecios = useMemo(() => {
    ...
    // Deduplicar: por cada producto, el menor costo unitario (por kg/und), no el bulto
    const mapa = new Map<string, PrecioProveedor>();
    const costoUnit = (p: PrecioProveedor) => p.precioCosto / (p.cantidadEmbalaje || 1);
    base.forEach(p => {
      const existente = mapa.get(p.productoId);
      if (!existente || costoUnit(p) < costoUnit(existente)) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddOrUpdatePrecio({
      ...
      tipoEmbalaje: editingPrecio?.tipoEmbalaje,
      cantidadEmbalaje: editingPrecio?.cantidadEmbalaje,
    });
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredPrecios.map((precio) => {
                      ...
                      const costoUnit = precio.precioCosto / (precio.cantidadEmbalaje || 1);
                      const markup = costoUnit > 0
                        ? ((product.precioVenta - costoUnit) / costoUnit) * 100
                        : 0;
