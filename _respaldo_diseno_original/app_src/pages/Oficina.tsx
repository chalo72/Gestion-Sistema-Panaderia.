...
export default function Oficina({ publicAppUrl, onViewChange, ventas = [], productos = [], inventario = [], sesionesCaja = [], creditosClientes = [], gastos = [], formatCurrency }: OficinaProps) {
  ...
  const kpis = useMemo(() => {
    ...
    const ventasHoy = ventas.filter((v) => fechaCalendarioLocalDesdeISO(v.fecha) === hoy);
    const totalHoy = ventasHoy.reduce((s, v) => s + v.total, 0);

    const ayer = new Date();
    ayer.setDate(ayer.getDate() - 1);
    const ayerStr = fechaCalendarioLocalDesdeISO(ayer.toISOString());
    const totalAyer = ventas.filter((v) => fechaCalendarioLocalDesdeISO(v.fecha) === ayerStr).reduce((s, v) => s + v.total, 0);
