...
import { pullUsersFromCloud, mergeUsersToLocalStorage, notifyUsuariosSync } from '@/lib/user-cloud-sync';
...
export function Login({ onLoginSuccess }: LoginProps) {
  ...
  useEffect(() => {
    ...
    pullUsersFromCloud()
      .then((remote) => {
        if (remote.length > 0) {
          const cambios = mergeUsersToLocalStorage(remote);
  ...
  const handlePinSubmit = async (finalPin: string) => {
    ...
    setIsLoading(true);
    try {
      let result = await login(selectedUser.email, finalPin);

      // Si falla, traer claves frescas de la nube e intentar una vez más
      if (!result.success) {
        try {
          const remote = await pullUsersFromCloud();
          const cambios = mergeUsersToLocalStorage(remote);
          if (cambios > 0) notifyUsuariosSync('pull_nube');
          result = await login(selectedUser.email, finalPin);
        } catch {
