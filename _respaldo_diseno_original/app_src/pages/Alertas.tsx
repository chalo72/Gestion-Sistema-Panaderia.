...
function calcPrioridad(pct: number, _tipo: 'subida' | 'bajada'): 'CRÍTICA' | 'ALTA' | 'NORMAL' {
  // Mismos umbrales para subidas (riesgo) y bajadas (oportunidad)
...
export function Alertas({
  ...
  const handleClearAll = () => {
    ...
    if (leidas.length === 0) {
      toast.message('No hay alertas en el historial para borrar');
      return;
    }
    if (confirm(`¿Eliminar ${leidas.length} alerta(s) del historial? Las pendientes se conservan.`)) {
      leidas.forEach((a) => onDeleteAlerta(a.id));
