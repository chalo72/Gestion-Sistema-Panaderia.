...
import { iniciarDictadoVoz } from '@/lib/dictado-voz';
...
export default function AgentesIA() {
  ...
  // ── Voice Recording (iniciarDictadoVoz continuo con auto-reinicio) ──
  const [isRecording, setIsRecording] = useState(false);
  const dictadoControlRef = useRef<ReturnType<typeof iniciarDictadoVoz> | null>(null);

  const toggleRecording = () => {
    if (isRecording && dictadoControlRef.current) {
      dictadoControlRef.current.detener();
      dictadoControlRef.current = null;
      setIsRecording(false);
      toast.info('🎙️ Micrófono pausado');
      return;
    }

    const control = iniciarDictadoVoz({
      textoInicial: comando,
      onTranscript: (estado) => {
        setComando(estado.completo);
      },
      onEstado: (escuchando) => {
        setIsRecording(escuchando);
      },
      onError: (err) => {
    ...
    if (!control) {
      toast.error('Tu navegador no soporta reconocimiento de voz nativo o el micrófono está bloqueado.');
      return;
    }

    dictadoControlRef.current = control;
    setIsRecording(true);
    toast.info('🎙️ Escuchando... habla con tranquilidad');
  };
  ...
  const ejecutarComandoJarvis = async () => {
    if (!comando.trim() || ejecutando || isRecording) return;
    
    const id = ++sesionIdRef.current;
    const comandoFinal = comando.trim();
    setComando('');
    setEjecutando(true);
    setTextoGerente('');
    ...
    try {
      const { ejecutarSwarmJarvis } = await import('@/lib/jarvis-core/swarm-orchestrator');
      const response = await ejecutarSwarmJarvis(comandoFinal, (msg) => {
         setTextoGerente(msg);
      }, db); // <-- PASSING DB HERE

      setSesiones(prev => prev.map(s => s.id === id ? {
         ...s, 
         analisisGerente: response.sintesisNexus,
         completado: true,
         tareas: response.agentesUsados.map(ag => ({ agente: ag as any, tarea: 'Análisis de Enjambre completado', estado: 'done' as any, requiereAprobacion: false }))
      } : s));
    } catch (err: any) {
      console.error(err);
      toast.error('Error JARVIS: ' + (err.message || err));
      setSesiones(prev => prev.map(s => s.id === id ? { ...s, completado: true, analisisGerente: 'Fallo crítico en el enjambre: ' + err.message} : s));
    } finally {
      setEjecutando(false);
      setTextoGerente('');
    }
              ...
              <div className="relative bg-black/90 border border-white/10 rounded-2xl p-3 flex items-end gap-3 shadow-2xl transition-all focus-within:border-[#DAA520]/40">
                ...
                <div className="flex gap-2 shrink-0 pr-1 pb-1">
                  ...
                  >
                    ...
                    <Button
                      onClick={() => ejecutarComandoJarvis()}
                      disabled={!comando.trim() || ejecutando || isRecording}
                      className="h-[52px] px-6 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black uppercase text-sm tracking-widest shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:scale-105 transition-all"
                      title="Ejecutar Arquitectura Enjámbrica"
                    >
                      {ejecutando ? <Loader2 className="w-6 h-6 animate-spin" /> : <>J.A.R.V.I.S. <BrainCircuit className="w-5 h-5 ml-2" /></>}
                    </Button>
                </div>
              </div>
            </div>
