...
export default function Dashboard(props: DashboardProps) {
  ...
  const forceSyncMobile = async () => {
    toast.loading('Forzando sincronizacin con el computador...', { id: 'sync-mobile' });
    try {
        localStorage.removeItem('formulaciones');
        localStorage.removeItem('modelosPan');
        if (navigator.serviceWorker) {
            const regs = await navigator.serviceWorker.getRegistrations();
            for (let r of regs) { await r.unregister(); }
        }
        await db.adapter.deleteDocument('backups', 'formulaciones_data').catch(()=>null);
        await db.adapter.deleteDocument('backups', 'modelosPan_data').catch(()=>null);
        toast.success('Sincronizacin forzada! Reiniciando...', { id: 'sync-mobile' });
        setTimeout(() => window.location.reload(), 1500);
    } catch(e) {
        toast.error('Error forzando sync', { id: 'sync-mobile' });
    }
  };
  ...
  return (
    ...
    <div className="space-y-5 sm:space-y-8 pb-12">
        ...
        {/* BOTON DE SINCRONIZACION REMOVIDO PARA MANTENER LA LIMPIEZA VISUAL */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-10">
        ...
        <div className="space-y-6">
          <GlassCard className={cn(
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold">Alertas de Precios</h3>
          ...
          <div className="bg-slate-900 dark:bg-slate-800 p-4 sm:p-6 rounded-[2.5rem] border border-white/10 shadow-2xl">
            ...
            <div className="space-y-2">
              {[
                ...
                { label: 'Alertas', icon: Bell, onClick: onViewAlertas },
              ].map((item, idx) => (
                <button
                  key={idx}
