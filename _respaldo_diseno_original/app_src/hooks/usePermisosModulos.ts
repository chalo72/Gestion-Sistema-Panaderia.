...
export const MODULOS_CONFIGURABLES: ModuloInfo[] = [
  ...
  { id: 'expedientes',      label: 'Exped. Empleadas',      seccion: 'General' },
...
export function guardarPermisos(permisos: PermisosModulos) {
  ...
  import('@/lib/permisos-cloud-sync').then(({ pushPermisoBlob, MODULOS_KEY }) => {
    pushPermisoBlob(MODULOS_KEY, permisos).catch(() => {});
  });
}
