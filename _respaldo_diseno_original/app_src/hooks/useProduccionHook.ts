  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        // Persistir fusión localmente; la nube se actualiza solo si esta copia es más completa/reciente
        // (evita que un cliente vacío/viejo pise la formulación buena)
        const cloudFormCount = formCloud.length;
        const cloudModCount = modCloud.length;
        const totalIngredientes = (list: any[]) =>
          list.reduce((acc, f) => acc + getIngCount(f), 0);
        const cloudFormIngCount = totalIngredientes(formCloud);
        const finalFormIngCount = totalIngredientes(finalFormulaciones);

        // NUNCA subir a la nube si la lista resultante tiene menos ingredientes que los ya presentes en la nube
        const debeSubirForm =
          finalFormulaciones.length > 0 &&
          (cloudFormCount === 0 || finalFormulaciones.length >= cloudFormCount) &&
          finalFormIngCount >= cloudFormIngCount &&
          finalFormIngCount >= 20;

        const debeSubirMod =
          finalModelos.length > 0 &&
          (cloudModCount === 0 || finalModelos.length >= cloudModCount) &&
          finalModelos.length >= 10;

        if (debeSubirForm) db.saveBackup('formulaciones_data', finalFormulaciones).catch(() => {});
        if (debeSubirMod) db.saveBackup('modelosPan_data', finalModelos).catch(() => {});
        localStorage.setItem('formulaciones', JSON.stringify(finalFormulaciones));
        localStorage.setItem('modelosPan', JSON.stringify(finalModelos));
        // Espejo en IndexedDB para que la próxima apertura coincida
        Promise.all([
          ...finalFormulaciones.map((f) => db.updateFormulacion(f).catch(() => {})),
          ...finalModelos.map((m) => db.updateModeloPan(m).catch(() => {})),
        ]).catch(() => {});
