...
import { Calculator, PieChart, ArrowRight, Wand2, PlusCircle, CheckCircle2, AlertTriangle, Layers3, Flame, Trash2, Plus } from 'lucide-react';
...
interface DistribuidorArrobaProps {
  ...
  modelos: ModeloPan[];
  ventas: Venta[];
  onConfirmarProduccion?: (cortes: { modeloId: string; cantidad: number; pesoCrudoTotal: number }[], formId: string, arrobas: number) => void;
...
export function DistribuidorArroba({ formulaciones, modelos, ventas, onConfirmarProduccion, onSimulationChange, onGuardarAuditoria }: DistribuidorArrobaProps) {
  ...
  const pesoUtilizadoGr = useMemo(() => {
    return Object.entries(cortes).reduce((acc, [modId, cant]) => {
      ...
      return acc + (cant * mod.pesoUnitarioGr);
    }, 0);
  }, [cortes, modelos]);
  ...
  const handleConfirmar = () => {
    if (estaSobregirado) {
      toast.error('¡Estás intentando fabricar más pan del que la masa permite!');
      return;
    }
    if (pesoUtilizadoGr === 0) {
      toast.error('Debes asignar al menos un pan a la producción');
      return;
    }
    ...
    const payload = Object.entries(cortes)
      ...
      .map(([modId, cant]) => {
        ...
        return {
          modeloId: modId,
          cantidad: cant,
          pesoCrudoTotal: cant * (m?.pesoUnitarioGr || 0)
        };
      });

    if (onConfirmarProduccion) {
      onConfirmarProduccion(payload, formId, arrobas);
    } else if (!onGuardarAuditoria) {
      toast.success('Distribución confirmada (Simulación)');
    }
  };
    ...
    <Card className="border-0 shadow-none bg-transparent">
      <CardHeader className="px-0 pt-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-xl">Distribuidor Inteligente de Arroba</CardTitle>
            <CardDescription>Reparte una masa base en múltiples tipos de pan (Croissant, Mogolla, etc.)</CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        {formId && modelosHijos.length > 0 && (
          <div className="animate-in slide-in-from-bottom-4 duration-500 fade-in space-y-6">
            ...
            <div className="space-y-4">
              ...
              {/* Botones directos para agregar panes en lugar de Dropdown */}
              <div className="flex flex-wrap gap-2 mb-4">
                {modelosHijos.filter(m => cortes[m.id] === undefined).map(m => (
                  <Button 
                    ...
                    variant="outline" 
                    size="sm"
                    className="h-8 text-xs rounded-xl border-dashed border-slate-300 bg-white text-slate-600 hover:bg-slate-50 hover:text-indigo-600"
                  >
                    <Plus className="w-3 h-3 mr-1" /> {m.nombre}
                  </Button>
                ))}
              </div>
                      ...
                      <div className="flex flex-col gap-2 mb-3">
                        <div className="flex items-center gap-2">
                          <Input 
                            ...
                            placeholder="0"
                            className="h-12 rounded-xl text-center font-black text-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/20"
                          />
                          <span className="text-xs font-bold text-slate-400">panes</span>
                        </div>
            ...
            <div className="pt-4 flex flex-col sm:flex-row justify-end gap-3">
              {onGuardarAuditoria && (
                ...
                >
                  <Wand2 className="w-5 h-5 mr-2 text-violet-500" /> Auditar Producción (IA)
                </Button>
              )}
              {onConfirmarProduccion && (
                <Button 
                  onClick={handleConfirmar} 
                  disabled={pesoUtilizadoGr === 0 || estaSobregirado}
                  className={cn(
                    "h-14 px-8 rounded-2xl font-black text-lg transition-all",
                    estaSobregirado ? "bg-rose-500 hover:bg-rose-600 text-white" : "bg-indigo-600 hover:bg-indigo-700 text-white shadow-xl shadow-indigo-500/20"
                  )}
                >
                  {estaSobregirado ? (
                    <><AlertTriangle className="w-5 h-5 mr-2" /> Sobregiro de Masa</>
                  ) : (
                    <><Flame className="w-5 h-5 mr-2" /> Enviar a Producción</>
                  )}
                </Button>
              )}
            </div>
