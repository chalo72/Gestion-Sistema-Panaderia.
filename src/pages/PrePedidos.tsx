...
export default function PrePedidos({
  ...
  const [showMobileCart, setShowMobileCart] = useState(false); // Ticket como panel deslizable en celular (estilo Venta Rápida)
...
<body>
  ...
  return (
    <div className="h-[100dvh] w-full flex bg-slate-50 dark:bg-slate-950 overflow-hidden font-sans">
      ...
      {/* ═══ FONDO OSCURO AL DESLIZAR EL TICKET EN CELULAR ═══ */}
      {showMobileCart && (
        <div
          className="lg:hidden fixed inset-0 bg-black/50 z-40 animate-ag-fade-in"
          onClick={() => setShowMobileCart(false)}
        />
      )}

      {/* ═══ PANEL DERECHO (TICKET / CUENTA) — sidebar fijo en desktop, panel deslizable en celular ═══ */}
      <div
        className={cn(
          'fixed inset-x-0 bottom-0 z-50 h-[85vh] rounded-t-3xl border-t shadow-2xl transition-transform duration-300 ease-out flex flex-col bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800',
          showMobileCart ? 'translate-y-0' : 'translate-y-full',
          'lg:static lg:inset-auto lg:z-auto lg:translate-y-0 lg:h-full lg:w-[380px] xl:w-[420px] lg:rounded-none lg:border-t-0 lg:border-l lg:shadow-none lg:shrink-0'
        )}
        style={{ paddingBottom: 'max(env(safe-area-inset-bottom, 0px), 0px)' }}
      >

          {/* Agarradera visual del panel deslizable — solo celular */}
          <button
            onClick={() => setShowMobileCart(false)}
            className="lg:hidden w-full flex flex-col items-center gap-1 pt-2 pb-1 shrink-0 bg-[#1a1c2e]"
      ...
      {/* ═══ NAVEGACIÓN MÓVIL: pastilla flotante estilo Venta Rápida (un solo dedo) ═══ */}
      {!verDetallePedido && (
        <div
          className="lg:hidden fixed bottom-4 left-4 right-4 rounded-3xl overflow-hidden flex border-2 border-indigo-500 shadow-[0_8px_30px_rgba(0,0,0,0.3)] bg-white dark:bg-slate-900 z-[45]"
          style={{ paddingBottom: 'max(env(safe-area-inset-bottom, 0px), 0px)' }}
        >
          <button
            onClick={() => setShowMobileCart(false)}
          ...
          <button
            ...
            className={cn(
              'flex-[2] flex items-center justify-center gap-2 py-4 transition-all',
              showMobileCart || (activeDraft && activeDraft.items.length > 0)
            ...
            {activeDraft && activeDraft.items.length > 0 ? (
              <>
                ...
                <div className="flex flex-col items-start leading-none">
                  <span className="text-[11px] font-black">{formatCurrency(activeDraft.total)}</span>
                  <span className="text-[9px] opacity-70 font-bold uppercase">Ver pedido</span>
                </div>
              </>
            ) : (
              <>
